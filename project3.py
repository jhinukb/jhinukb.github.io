...
# 
# **Checkpoint.** For full credit, you must also **complete Part 1 of the project (out of 4) and submit them by 11:59pm on Friday 11/21**. You will have some lab time to work on these questions, but we recommend that you start the project before lab and leave time to finish the checkpoint afterward.
# 
...
unstemmed_run = vocab_table.where('Stem', 'run').column(1)
unstemmed_run
...
most_shortened = vocab_table.column(0).item(np.where(shortened == max(shortened))[0].item(0)) #shortened.index(min(shortened))
print(most_shortened)
...
def romance_proportion(table):
    ...
    return np.count_nonzero(table.column('Genre') == 'romance')

# The staff solution took 4 lines.  Start by creating a table.
tbl_prop = Table().with_columns(
    'Datasets', make_array('Train', 'Test'),
    'Romance Proportion', make_array(romance_proportion(train_movies), romance_proportion(test_movies))
)
print(tbl_prop)
tbl_prop.barh('Datasets', 'Romance Proportion')
...
movie_with_distance = train_movies.with_column('distance', train_movies.apply(distance_from_batman_returns, 'Title'))
...
my_20_features = make_array('power', 'home', 'data', 'control', 'human', 'command', 'men', 'move', 'three', 'marri', 'captain', 'nice', 'play', 'huh', 'mother', 'mom', 'wife', 'boy', 'miss', 'run') 

train_20 = train_movies.select(my_20_features)
test_20 = test_movies.select(my_20_features)
# print(train_movies.num_rows)
# print(train_20.num_rows)
# print(test_movies.num_rows)
#print(test_20)
...
# I selected the features that were farthest from the line because it will be easier to distinguish if the word is more common in romance or action if the word is an extreme. I also chose common words instead of words like "weve" which is uncommon.
...
genre_and_distance_table = Table().with_columns(
    'Genre', train_movies.column('Genre'),
    'Distance', fast_distances(test_20.row(0), train_20)
)
genre_and_distances = genre_and_distance_table.sort("Distance")
genre_and_distances
...
my_assigned_genre = most_common('Genre', test_movies) #the mummy, action
...
def classify(test_row, train_rows, train_classes, k):
    ...
    #array of the fastest distances
    genre_and_distances_table = Table().with_columns(
        'Class', train_classes,
        'Distance', fast_distances(test_row, train_rows)
    )
    genre_and_distances = genre_and_distances_table.sort("Distance")
...
print(test_guesses)
print(test_movies.column('Genre'))
...
staff_features = make_array("come", "do", "have", "heart", "make", "never", "now", "wanna", "with", "yo", "nice", "marri", "power", "run", "home", "world", "captain")
...
# I used a higher value for k so the classifier will check more neighbors to classify the movie. This increases the chance that the classifier will be accurate because there are more values it is comparing with. The features were also different and more extreme features. They are common words that are in all of the movies in the test set.
